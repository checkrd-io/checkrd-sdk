...
pub struct TelemetryEventInput {
    ...
    // --- M (cost metering) additions (TDD App. A) -------------------------
    // Every one is an integer count, a micro-USD scalar, or a constrained
    // enum/code. Cost is computed in the WASM core from the signed pricing
    // bundle (ADR-002) — never customer content.
    /// PII: SAFE — OTel `gen_ai.operation.name`, constrained to the well-known
    /// operation enum (`chat`, `embeddings`, `generate_content`, …); validated
    /// at ingestion.
    #[serde(default)]
    pub gen_ai_operation: Option<String>,
    /// PII: SAFE — integer cache-read input token count (≥ 0); a subset of
    /// `gen_ai_input_tokens` after the OTel inclusion-rule normalization.
    #[serde(default)]
    #[typeshare(serialized_as = "Option<number>")]
    pub gen_ai_cache_read_input_tokens: Option<i64>,
    /// PII: SAFE — integer cache-creation input token count (≥ 0); same
    /// inclusion rule.
    #[serde(default)]
    #[typeshare(serialized_as = "Option<number>")]
    pub gen_ai_cache_creation_input_tokens: Option<i64>,
    /// PII: SAFE — integer reasoning output token count (≥ 0); a subset of
    /// `gen_ai_output_tokens`.
    #[serde(default)]
    #[typeshare(serialized_as = "Option<number>")]
    pub gen_ai_reasoning_output_tokens: Option<i64>,
    /// PII: SAFE — cost in integer micro-USD (≥ 0, or saturated with the
    /// overflow flag). Computed in the WASM core from the signed pricing bundle
    /// (ADR-002), never derived from customer content.
    #[serde(default)]
    #[typeshare(serialized_as = "Option<number>")]
    pub cost_usd_micros: Option<i64>,
    /// PII: SAFE — ISO 4217 alphabetic currency code; constant `"USD"` in v1.
    #[serde(default)]
    pub currency: Option<String>,
    /// PII: SAFE — pricing-bundle version the cost was computed against (a
    /// monotonic counter, not customer data). REQUIRED when `pricing_status`
    /// is `"priced"` (enforced at ingestion).
    #[serde(default)]
    #[typeshare(serialized_as = "Option<number>")]
    pub pricing_bundle_version: Option<u64>,
    /// PII: SAFE — constrained enum: `priced` | `unpriced_model` | `untallied`
    /// | `disabled`.
    #[serde(default)]
...
/// Pricing-status enum (TDD App. A). `priced` means a cost was computed
/// against a signed bundle; the other three record *why* a cost is absent.
/// `priced` is the only value that requires `pricing_bundle_version`.
const VALID_PRICING_STATUSES: &[&str] = &["priced", "unpriced_model", "untallied", "disabled"];

/// OTel `gen_ai.operation.name` registry values. Mirrors the well-known
/// operations the GenAI semantic conventions enumerate, so the field stays a
/// closed enum rather than free text.
const VALID_GEN_AI_OPERATIONS: &[&str] = &[
    "chat",
    "text_completion",
    "embeddings",
    "generate_content",
...
pub fn validate_telemetry_event(
    ...
    // --- M (cost metering) validation (TDD §4.5) -------------------------
    // Token sub-counts are non-negative integers, same guard style as the
    // input/output token checks above.
    if let Some(tokens) = event.gen_ai_cache_read_input_tokens {
        if tokens < 0 {
            return Err(TelemetryValidationError::InvalidValue {
                field: "gen_ai_cache_read_input_tokens",
                value: tokens.to_string(),
            });
        }
    }
    if let Some(tokens) = event.gen_ai_cache_creation_input_tokens {
        if tokens < 0 {
            return Err(TelemetryValidationError::InvalidValue {
                field: "gen_ai_cache_creation_input_tokens",
                value: tokens.to_string(),
            });
        }
    }
    if let Some(tokens) = event.gen_ai_reasoning_output_tokens {
        if tokens < 0 {
            return Err(TelemetryValidationError::InvalidValue {
                field: "gen_ai_reasoning_output_tokens",
                value: tokens.to_string(),
            });
        }
    }
    // Cost is a non-negative micro-USD scalar (saturated, never negative).
    if let Some(cost) = event.cost_usd_micros {
        if cost < 0 {
            return Err(TelemetryValidationError::InvalidValue {
    ...
    // Currency is an ISO 4217 alphabetic code: exactly 3 uppercase ASCII
    // letters. v1 is effectively constant "USD", but the check accepts any
    // well-formed code so a future multi-currency bundle doesn't need a
    // wire-format change.
    ...
    if let Some(ref status) = event.pricing_status {
        if !VALID_PRICING_STATUSES.contains(&status.as_str()) {
            return Err(TelemetryValidationError::InvalidValue {
    ...
    if let Some(ref op) = event.gen_ai_operation {
        if !VALID_GEN_AI_OPERATIONS.contains(&op.as_str()) {
            return Err(TelemetryValidationError::InvalidValue {
                field: "gen_ai_operation",
                value: op.clone(),
            });
        }
    }
    // Headline cross-field rule: a `priced` event MUST carry the bundle
    // version it was priced against, otherwise the cost is unauditable.
    if event.pricing_status.as_deref() == Some("priced") && event.pricing_bundle_version.is_none() {
        return Err(TelemetryValidationError::Required("pricing_bundle_version"));
    }
...
mod tests {
    ...
    fn valid_event_input() -> TelemetryEventInput {
        TelemetryEventInput {
            ...
            // Cost-metering fields populated so the bidirectional PII allowlist
            // test exercises every new serialized key. `priced` requires a
            // bundle version (M-7 ingestion rule), so both are set together.
            gen_ai_operation: Some("chat".into()),
            gen_ai_cache_read_input_tokens: Some(128),
            gen_ai_cache_creation_input_tokens: Some(0),
            gen_ai_reasoning_output_tokens: Some(64),
            cost_usd_micros: Some(4500),
            currency: Some("USD".into()),
            pricing_bundle_version: Some(7),
            pricing_status: Some("priced".into()),
    ...
    const ALLOWED_FIELD_NAMES: &[&str] = &[
        ...
        // M (cost metering) additions — counts, a micro-USD scalar, or
        // constrained enums/codes; cost is core-computed, never customer text.
        "gen_ai_operation",                   // SAFE: OTel operation enum
        "gen_ai_cache_read_input_tokens",     // SAFE: integer count
        "gen_ai_cache_creation_input_tokens", // SAFE: integer count
        "gen_ai_reasoning_output_tokens",     // SAFE: integer count
        "cost_usd_micros",                    // SAFE: integer micro-USD, core-computed
        "currency",                           // SAFE: ISO 4217 alpha code
        "pricing_bundle_version",             // SAFE: monotonic version counter
        "pricing_status",                     // SAFE: constrained enum
        // B1 additions:
    ...
    // M (cost metering) validation — TDD §4.5
    //
    // One adversarial test per rule: each rejects bad input AND accepts
    // good. `valid_event_input()` already carries a fully-populated,
    // self-consistent cost block (`priced` + bundle version 7), so each
    // test mutates exactly one field to isolate its rule.
    // -----------------------------------------------------------------

    #[test]
    fn rejects_negative_cache_read_input_tokens() {
        let mut ev = valid_event_input();
        ev.gen_ai_cache_read_input_tokens = Some(-1);
        assert!(validate_telemetry_event(&ev).is_err());
    }

    #[test]
    fn accepts_nonnegative_cache_read_input_tokens() {
        let mut ev = valid_event_input();
        ev.gen_ai_cache_read_input_tokens = Some(0);
        assert!(validate_telemetry_event(&ev).is_ok());
    }

    #[test]
    fn rejects_negative_cache_creation_input_tokens() {
        let mut ev = valid_event_input();
        ev.gen_ai_cache_creation_input_tokens = Some(-7);
        assert!(validate_telemetry_event(&ev).is_err());
    }

    #[test]
    fn accepts_nonnegative_cache_creation_input_tokens() {
        let mut ev = valid_event_input();
        ev.gen_ai_cache_creation_input_tokens = Some(42);
        assert!(validate_telemetry_event(&ev).is_ok());
    }

    #[test]
    fn rejects_negative_reasoning_output_tokens() {
        let mut ev = valid_event_input();
        ev.gen_ai_reasoning_output_tokens = Some(-3);
        assert!(validate_telemetry_event(&ev).is_err());
    }

    #[test]
    fn accepts_nonnegative_reasoning_output_tokens() {
        let mut ev = valid_event_input();
        ev.gen_ai_reasoning_output_tokens = Some(0);
        assert!(validate_telemetry_event(&ev).is_ok());
    }

    #[test]
    fn rejects_negative_cost_usd_micros() {
        let mut ev = valid_event_input();
        ev.cost_usd_micros = Some(-1);
        assert!(validate_telemetry_event(&ev).is_err());
    }

    #[test]
    fn accepts_nonnegative_cost_usd_micros() {
        let mut ev = valid_event_input();
        ev.cost_usd_micros = Some(0);
        assert!(validate_telemetry_event(&ev).is_ok());
    }
    ...
    fn rejects_lowercase_currency() {
        let mut ev = valid_event_input();
        ev.currency = Some("usd".into());
        assert!(validate_telemetry_event(&ev).is_err());
    }

    #[test]
    fn rejects_currency_wrong_length() {
        let mut ev = valid_event_input();
        ev.currency = Some("US".into());
        assert!(validate_telemetry_event(&ev).is_err());
        ev.currency = Some("USDD".into());
        assert!(validate_telemetry_event(&ev).is_err());
    }

    #[test]
    fn rejects_currency_non_alpha() {
        let mut ev = valid_event_input();
        ev.currency = Some("US1".into());
        assert!(validate_telemetry_event(&ev).is_err());
    }

    #[test]
    fn accepts_valid_currency() {
        // USD (v1 constant) and any other well-formed ISO 4217 alpha code.
        for code in ["USD", "EUR", "GBP", "JPY"] {
            let mut ev = valid_event_input();
            ev.currency = Some(code.into());
            assert!(validate_telemetry_event(&ev).is_ok(), "currency {code}");
        }
    }

    #[test]
    fn rejects_unknown_pricing_status() {
        let mut ev = valid_event_input();
        ev.pricing_status = Some("free".into());
        assert!(validate_telemetry_event(&ev).is_err());
    }

    #[test]
    fn accepts_all_pricing_statuses() {
        // `priced` keeps the bundle version from the base event; the other
        // three drop it (a bundle version is only mandatory for `priced`).
        for status in VALID_PRICING_STATUSES {
            let mut ev = valid_event_input();
            ev.pricing_status = Some((*status).into());
            if *status != "priced" {
                ev.pricing_bundle_version = None;
            }
            assert!(validate_telemetry_event(&ev).is_ok(), "status {status}");
        }
    }

    #[test]
    fn rejects_unknown_gen_ai_operation() {
        let mut ev = valid_event_input();
        ev.gen_ai_operation = Some("summarize".into());
        assert!(validate_telemetry_event(&ev).is_err());
    }

    #[test]
    fn accepts_all_gen_ai_operations() {
        for op in VALID_GEN_AI_OPERATIONS {
            let mut ev = valid_event_input();
            ev.gen_ai_operation = Some((*op).into());
            assert!(validate_telemetry_event(&ev).is_ok(), "operation {op}");
        }
    }

    // Headline cross-field rule: priced ⇒ bundle version. All four
    // combinations of (pricing_status == "priced") × (bundle version set).
    #[test]
    fn priced_with_bundle_version_ok() {
        let mut ev = valid_event_input();
        ev.pricing_status = Some("priced".into());
        ev.pricing_bundle_version = Some(7);
        assert!(validate_telemetry_event(&ev).is_ok());
    }

    #[test]
    fn priced_without_bundle_version_rejected() {
        let mut ev = valid_event_input();
        ev.pricing_status = Some("priced".into());
        ev.pricing_bundle_version = None;
        let err = validate_telemetry_event(&ev).unwrap_err();
        // Must reject specifically for the missing bundle version, not for an
        // unrelated field — pins the cross-field rule to the right cause.
        assert!(
            matches!(
                err,
                TelemetryValidationError::Required("pricing_bundle_version")
            ),
            "expected Required(pricing_bundle_version), got {err:?}"
        );
    }

    #[test]
    fn unpriced_without_bundle_version_ok() {
        let mut ev = valid_event_input();
        ev.pricing_status = Some("unpriced_model".into());
        ev.pricing_bundle_version = None;
        assert!(validate_telemetry_event(&ev).is_ok());
    }

    #[test]
    fn unpriced_with_bundle_version_ok() {
        // A bundle version on a non-priced event is allowed (e.g. the bundle
        // was loaded but the model wasn't in it); only `priced` mandates it.
        let mut ev = valid_event_input();
        ev.pricing_status = Some("untallied".into());
        ev.pricing_bundle_version = Some(7);
        assert!(validate_telemetry_event(&ev).is_ok());
    }

    // --- Boundary tests pin the strict comparisons in the validator so a
    // `< → <=` / `> → >=` / `|| → &&` mutation cannot survive. (These guards
    // predate Stream M but live in the validator M-7 extends, so they are held
    // to the same mutation-tested bar; surfaced by `cargo mutants`.) ---

    #[test]
    fn validate_accepts_zero_token_counts() {
        // A literal zero is valid — the guards are `< 0`, not `<= 0`.
        let mut ev = valid_event_input();
        ev.gen_ai_input_tokens = Some(0);
        ev.gen_ai_output_tokens = Some(0);
        ev.gen_ai_cache_read_input_tokens = Some(0);
        ev.gen_ai_cache_creation_input_tokens = Some(0);
        ev.gen_ai_reasoning_output_tokens = Some(0);
        ev.cost_usd_micros = Some(0);
        assert!(validate_telemetry_event(&ev).is_ok());
    }
    ...
    #[test]
    fn validate_accepts_both_policy_modes_rejects_other() {
        let mut ev = valid_event_input();
        for mode in ["enforce", "dry_run"] {
            ev.policy_mode = Some(mode.to_string());
            assert!(validate_telemetry_event(&ev).is_ok(), "mode {mode}");
        }
        ev.policy_mode = Some("shadow".to_string());
        assert!(validate_telemetry_event(&ev).is_err());
    }

    #[test]
    fn validate_evaluation_path_length_boundary() {
        ...
        let mut ev = valid_event_input();
        // Exactly 32 steps accepted; 33 rejected (pins `len > 32`, not `>= 32`).
        ev.evaluation_path = (0..32).map(|_| step()).collect();
        assert!(validate_telemetry_event(&ev).is_ok(), "32 steps");
        ev.evaluation_path = (0..33).map(|_| step()).collect();
        assert!(validate_telemetry_event(&ev).is_err(), "33 steps");
    }

    #[test]
    fn validate_evaluation_step_field_boundaries() {
        let mut ev = valid_event_input();
        // An empty stage is rejected — pins the `||` against `&&`.
        ev.evaluation_path = vec![EvaluationStep {
            stage: String::new(),
            rule: None,
            result: "x".into(),
            detail: None,
        }];
        assert!(validate_telemetry_event(&ev).is_err(), "empty stage");
        // Stage exactly 64 chars accepted; 65 rejected (pins `len > 64`).
        ev.evaluation_path = vec![EvaluationStep {
            stage: "a".repeat(64),
            rule: None,
            result: "x".into(),
        ...
        assert!(validate_telemetry_event(&ev).is_ok(), "64-char stage");
        ev.evaluation_path = vec![EvaluationStep {
            stage: "a".repeat(65),
            rule: None,
            result: "x".into(),
        ...
        assert!(validate_telemetry_event(&ev).is_err(), "65-char stage");
        // Rule exactly 128 chars accepted; 129 rejected (pins `len > 128`).
        ev.evaluation_path = vec![EvaluationStep {
            stage: "ok".into(),
            rule: Some("a".repeat(128)),
            result: "x".into(),
            detail: None,
        }];
        assert!(validate_telemetry_event(&ev).is_ok(), "128-char rule");
        ev.evaluation_path = vec![EvaluationStep {
            stage: "ok".into(),
            rule: Some("a".repeat(129)),
            result: "x".into(),
            detail: None,
        }];
        assert!(validate_telemetry_event(&ev).is_err(), "129-char rule");
    }
}
