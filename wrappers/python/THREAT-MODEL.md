...
| 14 | `checkrd_core.wasm` | Supply-chain tampering of the published binary | SHA-256 integrity verified on every engine construction; the embedding wheel carries Sigstore-signed PEP 740 attestations via PyPI Trusted Publishing; verification recipe (`pypi-attestations verify pypi …`) in [WASM-CORE.md § Integrity Verification](./WASM-CORE.md#integrity-verification) |
...
dev-time tool — engineers on a source checkout without a
regenerated ``_wasm_integrity.py`` need a way in. But a dev env
var silently leaking into a production deploy is exactly the
class of mistake that disables supply-chain verification without
anyone noticing.

**Mitigation.** Eleven common environment signals are checked
(``NODE_ENV``, ``ENVIRONMENT``, ``APP_ENV``, ``RAILS_ENV``,
``DJANGO_ENV``, ``PYTHON_ENV``, ``DEPLOYMENT_ENVIRONMENT``, etc.)
against four production values (``production``, ``prod``,
``canary``, ``live``). If any is set AND the skip flag is set,
the SDK refuses to load unless the operator types the exact
break-glass phrase
``CHECKRD_I_UNDERSTAND_WASM_INTEGRITY_RISK=i-understand-the-risk``.
The phrase is a deliberate muscle-memory barrier, not a
configuration knob — any tolerance for whitespace or uppercasing
would erode the barrier. See ``tests/test_engine.py::TestWasmIntegrityProductionGuard``.
...
engine is initialized. The Python host copy that ``bind_engine()``
consumes is a ``bytearray`` that the Rust core explicitly
zero-fills on drop. Python garbage-collection timing is
unpredictable — the caller-facing ``bytearray`` may linger
briefly after the WASM-side copy is zeroed, but callers are
strongly encouraged to use :class:`~checkrd.ExternalIdentity`
with a KMS backend for production deployments. Documented at
https://checkrd.io/docs/identity.
...
trails downstream. See ``tests/test_engine.py`` for the
detection suite.

**Mitigation.** The browser guard was tightened from a
too-broad ``!process.versions.node`` heuristic to a
real-browser detection (window + document + navigator.userAgent
present, NO Deno/Bun/WorkerGlobalScope/EdgeRuntime markers).
Even with the ``dangerouslyAllowBrowser`` opt-in, a loud
one-time stderr banner fires naming the forged-telemetry
attack specifically.
...
in logs. Banner is idempotent, thread-safe, and falls back to
``console.warn`` on runtimes without ``process.stderr``.

### Telemetry signing failure visibility

Before 0.3.0, a telemetry batch that failed to sign was dropped
with a single log line and the same counter as transient
network errors. That made sustained signing failures (a
misconfigured ``ExternalIdentity``, a rotated-away key) look
identical on the dashboard to an occasional ingestion flap.

**Mitigation.** Signing drops are counted separately in
``diagnostics()["dropped_signing_error"]`` and fire a separate
``on_drop("signing_error", count)`` callback. Operators can
page on ``signing_error > 0`` for any duration (configuration
error, demands a fix) without the noise from transient
``send_error`` spikes.
...
state-poll GET. Customers pin via ``api_version=`` or
...
fleet is still on SDK <0.3.0" or "are Python 3.9 callers
seeing a different error rate." A compromised transitive dep
that flipped the SDK identifier to a wrong value would have
been invisible.

**Mitigation.** ``X-Checkrd-SDK-Lang`` /
``-Version`` / ``-Runtime`` / ``-Runtime-Version`` / ``-OS`` /
``-Arch`` are stamped on every control-plane request. The
Stainless ``X-Stainless-*`` family is the industry reference.
