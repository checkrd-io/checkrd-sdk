...
class AlertRule:
    ...
    `POST /v1/alerts/{id}/unmute`.
        ...
        Attributes:
            id (UUID):
            agent_id (UUID):
            condition_type (str): Logical condition family (e.g., `error_rate`, `latency_p99`,
                `denied_rate`, `volume_drop`).
            threshold (float):
            window_minutes (int):
            channel (str): Notification channel: `email` or `webhook`.
            channel_target (str): Channel target: email address or webhook URL.
            cooldown_minutes (int):
            notification_count (int):
            is_enabled (bool):
            created_at (datetime.datetime):
            alert_state (str): Current alert state: `ok`, `no_data`, `pending`, `firing`,
                `resolved`.
