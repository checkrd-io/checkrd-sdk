...
| 14 | `checkrd_core.wasm` | Supply-chain tampering of the published binary | SHA-256 integrity verified on every engine construction; the embedding npm package carries Sigstore-signed npm provenance via `npm publish --provenance`; verification recipe (`npm audit signatures`) in [WASM-CORE.md § Integrity Verification](./WASM-CORE.md#integrity-verification) |
