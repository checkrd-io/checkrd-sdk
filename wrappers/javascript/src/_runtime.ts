/**
 * Checkrd lean runtime — `init` / `wrap` / `shutdown` / health, with ZERO
 * dependency on the six vendor instrumentors or the webhook verifier.
 *
 * # Why this module exists (bundle size)
 *
 * The edge + framework adapters (`checkrd/cloudflare`, `checkrd/next`,
 * `checkrd/hono`, `checkrd/ai-sdk`) only ever need the init / wrap
 * surface — they never call `instrument*()`. Those adapters used to
 * import from the umbrella `./index.js`, which ALSO re-exports the six
 * vendor instrumentors (via the `Checkrd` class → `client.js`) and the
 * webhook verifier. esbuild could not reliably drop that transitive
 * weight across the `index` ↔ `client` re-export cycle, so every edge
 * adapter shipped tens of kB of unrelated code (`verifyWebhook`, the
 * `Checkrd` class, the error/logger re-export surface).
 *
 * Splitting the runtime out means the adapters import ONLY the init
 * path (`initAsync`, `wrapAsync`, `shutdown`, `getSink`) plus its
 * genuine dependencies (WASM engine, telemetry batcher, control
 * receiver). `index.ts` re-exports every symbol below verbatim, so the
 * public API surface is unchanged — this is a pure internal split.
 */
...
export interface InitOptions {
  ...
  /**
   * Opt in to per-call cost metering (M-12). DEFAULT OFF. Also enabled by
   * the env var ``CHECKRD_COST_METERING=1``. When on AND a signed pricing
   * bundle has been installed, each telemetry event is enriched with the
   * call's settled cost (``cost_usd_micros`` / ``currency`` /
   * ``pricing_bundle_version`` / ``pricing_status``), computed in-WASM
   * inside the signed batch (ADR-002) — an independently verifiable spend
   * record, not a wrapper-side estimate. The whole path is inert when off
   * or when no pricing bundle is present.
   */
...
function completeInit(
  ...
  // Cost metering (M-12): DEFAULT OFF. Enabled by the explicit option OR
  // the env var `CHECKRD_COST_METERING=1`. The env read goes through the
  // cross-runtime-safe `readEnv` accessor (returns undefined on edge /
  // browser), so this never touches `process` directly and stays
  // edge-clean. Only exact "1" counts as truthy, matching the
  // security-flag convention in `_settings.ts`.
  const costMetering =
    (options.costMetering ?? false) || readEnv("CHECKRD_COST_METERING") === "1";
