...
no filesystem, no syscalls. The host wrapper (this JavaScript SDK, or
the Python SDK) does all I/O and passes byte buffers across the FFI
boundary.
...
```
  ┌─────────────────────────────────────────────────────┐
  │  Host process (Node 18+ / Bun / Deno)               │
  │                                                     │
  │   ┌───────────────────┐     ┌───────────────────┐   │
  │   │ fetch transport   │     │ TelemetryBatcher  │   │
  │   │ ControlReceiver   │     │ sinks             │   │
  │   └────────┬──────────┘     └────────┬──────────┘   │
  │            │ FFI calls (bytes in / bytes out)       │
  │  ──────────┼────────────────────────────────────────│ ← trust boundary
  │            ▼                                        │
  │   ┌─────────────────────────────────────────────┐   │
  │   │  checkrd_core.wasm (WebAssembly.Instance)   │   │
  │   │  ─────────────────────────────────────────  │   │
  │   │  • policy evaluation                        │   │
  │   │  • rate-limit counters                      │   │
  │   │  • kill-switch state                        │   │
...
The core runs inside a `WebAssembly.Instance` owned by the host
wrapper. Per the WebAssembly specification, the module has no raw
access to system calls, no filesystem access, and no network access.
All interaction with the outside world is done through the FFI
exports listed below. Linear memory accesses are bounds-checked at
the region level; out-of-bounds reads or writes trap the module
without corrupting host memory.

The WASM binary is instantiated via `new WebAssembly.Instance(module,
{ wasi_snapshot_preview1: wasi.wasiImport })` using the
`@bjorn3/browser_wasi_shim` package to satisfy the WASI imports
(`random_get`, `fd_write`) that the Rust standard library requires
even when no I/O is performed.

Each `WasmEngine` instance is backed by a separate
`WebAssembly.Instance` — rate limiters, kill switches, policy state,
and signing keys are isolated per instance. Constructing two
`WasmEngine` objects yields two independent sandboxes that cannot
observe or interfere with each other.

The Rust source is compiled to `wasm32-wasip1` with `opt-level = 3`,
`lto = "thin"`, and `panic = "abort"`. Panics inside the core abort
the WASM instance; they do not throw into the host.
...
error class `.code` strings; the wire values MUST NOT change within
a major version.
...
WASM linear memory is isolated per instance. The host reads and
writes it through `new Uint8Array(instance.exports.memory.buffer,
ptr, len)`.
...
4. **Aliasing.** The same `(ptr, len)` MUST NOT be passed to two
   different exports concurrently. JavaScript's single-threaded event
   loop serializes FFI calls per instance by construction; hosts using
   Worker threads MUST give each worker its own `WasmEngine`.

Secrets — private keys and signed payloads — live in linear memory
for the lifetime of the instance. On instance drop, the
`WebAssembly.Memory` buffer is eventually garbage-collected; the
Rust source additionally zeroizes sensitive buffers via
`zeroize::ZeroizeOnDrop` before drop. Host callers MUST drop the
engine before the surrounding process exits if process memory may be
captured (e.g., core dumps) outside their trust boundary.
...
- Cross-implementation interop against the Python SDK's
  PyCA `cryptography`-backed verifier (signing path and
...
2. **Entropy.** `generate_keypair` draws 32 bytes from the host's
   WASI `random_get` implementation. In this SDK,
   `@bjorn3/browser_wasi_shim` maps `random_get` to
   `globalThis.crypto.getRandomValues` — the cryptographically-
   secure entropy source mandated by the Web Crypto specification.
   All other exports are deterministic.

Replaying the same `(policy, request, timestamp)` triple through
`evaluate_request` returns a byte-identical result.

## Side-Channel Posture

The Ed25519 implementation is designed to be constant-time on native
targets. When compiled to `wasm32-wasip1` and executed under the
host's WebAssembly engine (V8, JavaScriptCore, SpiderMonkey, or
wasmtime via Bun/Deno), data-dependent timing may be reintroduced
by:

1. The runtime's JIT code generation — V8 and wasmtime do not
   guarantee constant-time lowering of all operations.
...
- Deploy in the self-hosted mode with a hardened wasmtime build
  (via Bun or Deno with a custom runtime, or a dedicated Node build
  with `--experimental-vm-modules` disabled), or
...
running from a tagged commit. The published `npm` package carries
[npm provenance](https://docs.npmjs.com/generating-provenance-statements):
a Sigstore-signed attestation, generated by `npm publish
--provenance`, that records the source commit, build workflow, and
runner that produced the artifact. The provenance bundle is logged in
the public Sigstore Rekor transparency log.

A SLSA-aligned `actions/attest-build-provenance` step (uploading
attestations to GitHub directly, alongside npm provenance) is on the
roadmap. It is currently disabled because GitHub gates the API on
Enterprise plans for private orgs.
...
removes the only supply-chain defense on the WASM binary.

### Independent verification

Verify the published package against its npm provenance attestation
without trusting the package's self-declared digest:

```bash
# Verify provenance + integrity for every signed dependency in the
# install tree, including checkrd. Fails fast on any signature
# mismatch.
...
The bundle records the source commit, build workflow path, and
runner; npm rejects packages whose Sigstore certificate does not
match the publishing repo. A clean `npm audit signatures` exit
confirms every dependency in the install tree (including `checkrd`)
shipped with a valid Sigstore-signed provenance bundle.

### Async loader (edge runtimes)

For Cloudflare Workers, Vercel Edge, Deno, and the browser, the SDK
ships an asynchronous loader that avoids every Node-specific
module:

```ts
import { WasmEngine } from 'checkrd';

const engine = await WasmEngine.create(policyJson, agentId, {
  // Omit `wasm` to use fetch(new URL('../checkrd_core.wasm', import.meta.url)).
  // Pass an explicit source when the bundler does not resolve that
  // URL to a fetchable asset: a `URL`, a URL string, a `Response`, a
  // `Uint8Array`, an `ArrayBuffer`, or a pre-compiled WebAssembly.Module.
  wasm: myWasmBinding,
});
```

Under the hood the async path:

1. Loads bytes via `fetch(...)` + `response.arrayBuffer()` (or
   accepts the caller's `Uint8Array` / `ArrayBuffer` directly).
2. Verifies SHA-256 via `globalThis.crypto.subtle.digest`.
3. Compiles via `WebAssembly.compile`.
4. Instantiates inside the same WASI shim the sync path uses.

The `CHECKRD_SKIP_WASM_INTEGRITY=1` bypass and the SHA-256 pin in
`_wasm_integrity.ts` apply to both paths. Passing a pre-compiled
`WebAssembly.Module` directly skips the integrity check — callers
taking that path must verify bytes out-of-band themselves.

### CycloneDX SBOM

A CycloneDX SBOM is published with every release via the
`cyclonedx-npm` script (`npm run sbom`). For the JavaScript SDK it
is attached to the GitHub Release as `sbom.cdx.json`. The SBOM
...
- **Rollback of `reload_policy` (unsigned) is possible by design.**
  ...
  invariant.
- **Signed policy-bundle installation from the SSE control stream is
  not yet wired up in the JavaScript SDK.** The `policy_updated`
  control event is currently acknowledged with a log warning and no
  engine action. The `reload_policy_signed` FFI export is fully
  functional and is exercised by the unit tests; connecting the SSE
  pipeline to that export is tracked in `GAPS.md`.
