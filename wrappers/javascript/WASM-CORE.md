...
```
https://github.com/checkrd-io/checkrd-sdk/.github/workflows/publish-javascript.yml@refs/tags/v<VERSION>
```

For the Python SDK the identity is the same repository with
`publish-python.yml`.

npm releases carry
[npm provenance](https://docs.npmjs.com/generating-provenance-statements)
attestations, visible on the package page and programmatically via
`npm view checkrd --json | jq .attestations`.
...
without trusting the npm package's self-declared digest, use
[`cosign`](https://docs.sigstore.dev/cosign/) against the release
artifact:

```bash
# Extract the bundled WASM from the installed package
cp "$(node -p "require.resolve('checkrd/package.json').replace('/package.json','/checkrd_core.wasm')")" \
  checkrd_core.wasm

# Fetch the attestation bundle for the installed version
VERSION=$(node -p "require('checkrd/package.json').version")
gh attestation download --repo checkrd/checkrd \
  --digest "sha256:$(shasum -a 256 checkrd_core.wasm | awk '{print $1}')"
...
cosign verify-blob-attestation \
  ...
  --certificate-identity-regexp="^https://github.com/checkrd-io/checkrd-sdk/.github/workflows/publish-javascript.yml" \
  checkrd_core.wasm
```
